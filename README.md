...
### AgentEvaluator
Evaluates tool-calling correctness for a small LangChain agent (weather, temperature, and population lookup tools) - a fundamentally different testing problem from text-in/text-out evaluation. Checks tool selection correctness, argument correctness, call-sequence correctness for multi-tool queries, and whether the agent's final answer is actually grounded in the tool results it received (catches hallucinated numbers even when the right tool was called correctly).
